...
public class NotificationUtil {
    ...
    public static boolean checkOffsetInsideAvailableData(Integer start, Integer end, NwdafEventEnum eventEnum) {
        OffsetDateTime startOffset = OffsetDateTime.now().minusSeconds(start);
        OffsetDateTime endOffset = null;
        if (end != null && end > 0) {
            if (end > start) {
                return false;
            }
            endOffset = OffsetDateTime.now().minusSeconds(end);
        }
        List<OffsetDateTime> availableTimeStamps = null;
        switch (eventEnum) {
            case NF_LOAD:
                availableTimeStamps = nfLoadTimeStamps;
                break;
            case UE_MOBILITY:
                availableTimeStamps = ueMobilityTimeStamps;
                break;
            case UE_COMM:
                availableTimeStamps = ueCommunicationTimeStamps;
                break;
            default:
                break;
        }
        if (availableTimeStamps != null) {
            ...
            boolean withinRange = (last != null && startOffset.isBefore(last)) && (endOffset == null || (first != null && endOffset.isAfter(first)));
            int startIndex = Collections.binarySearch(availableTimeStamps, startOffset.withSecond(0).withNano(0));
            if (!withinRange || startIndex < 0) {
                return false;
            }
            if (endOffset == null) {
                for (int i = startIndex; i > 0; i--) {
                    if (availableTimeStamps.get(i).minusMinutes(2).isBefore(availableTimeStamps.get(i - 1))) {
                        return false;
                    }
                }
            } else {
                int endIndex = Collections.binarySearch(availableTimeStamps, endOffset.withSecond(0).withNano(0));
                if (endIndex < 0) {
                    return false;
                }
...
public static NnwdafEventsSubscriptionNotification getNotification(NnwdafEventsSubscription sub, Integer index,
    ...
    Integer no_secs = findOffset[0] != Integer.MIN_VALUE ? findOffset[0] : Constants.MIN_PERIOD_SECONDS;
...
//    if (!checkOffsetInsideAvailableData(no_secs, null, eType)) {
//        return null;
//    }
...
public static Integer[] waitForDataProducer(NwdafEventEnum requestedEvent, WakeUpMessage wakeUpMessage) throws InterruptedException, IOException {
...
public static List<Boolean> checkCanServeSubscriptions(Integer noValidEvents,
    ...
    for (int i = 0; i < noValidEvents; i++) {
        ...
        try {
            Integer[] wakeUpResult = waitForDataProducer(eType, wakeUpMessage);
...
public static WakeUpMessage wakeUpDataProducer(KafkaProducer kafkaProducer, NwdafEventEnum eType, Integer no_secs) {
